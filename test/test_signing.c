...
int test_signing() {
	...
	*/
	...
	hawkc_context_set_password(&ctx,(unsigned char*)"test", (size_t)4);
	hawkc_context_set_algorithm(&ctx,HAWKC_SHA_1);
	...
	*/
	...
	hawkc_validate_hmac(&ctx,&is_valid);
...
/*
	...
	*/
	...
	hawkc_context_init(&ctx);
	hawkc_context_set_password(&ctx,(unsigned char*)"w7*0T6C.0b4C#", (size_t)13);
...
int test_hawk_capatibility() {
	...
	hawkc_context_init(&ctx);
	hawkc_context_set_password(&ctx,(unsigned char*)pwd, strlen(pwd));
		hawkc_context_set_algorithm(&ctx,HAWKC_SHA_1);
	...
	EXPECT_BYTE_EQUAL(ctx.header_in.id.data, "1" , (int)ctx.header_in.id.len);
